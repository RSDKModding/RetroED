...
SceneEditorv5::SceneEditorv5(QWidget *parent) : QWidget(parent), ui(new Ui::SceneEditorv5)
{
    ...
    for (int f = 0; f < 8; ++f) {
        ...
        connect(filterToggles[f], &QCheckBox::toggled, [this, f] {
            viewer->sceneFilter ^= (1 << f);
    ...
    connect(ui->stampList, &QListWidget::currentRowChanged, [this](int c) {
        ...
        ui->stpFlipX->setDisabled(c == -1);
        ui->stpFlipY->setDisabled(c == -1);

        if (c == -1 || c >= viewer->stamps.stampList.count())
            return;

        viewer->selectedStamp = c;
        ui->stpName->setText(ui->stampList->currentItem()->text());
    });
    ...
    connect(ui->stpFlipX, &QCheckBox::toggled, [this](bool c) { viewer->stampFlip.x = c; });
    connect(ui->stpFlipY, &QCheckBox::toggled, [this](bool c) { viewer->stampFlip.y = c; });

    connect(ui->stpName, &QLineEdit::textChanged, [this](QString s){
        int c = ui->stampList->currentRow();
        ui->stampList->currentItem()->setText(s);
        viewer->stamps.stampList[c].name = s;
    });
    ...
    auto reSyncGameObject = [this, linkGameObject](bool syncGC, int oldListCount) {
        ...
        for (int o = viewer->entities.count() - 1; o >= 0; --o) {
            ...
            if (entity.type >= objOffset){
                ...
                if (entity.type == objOffset && stageConfig.loadGlobalObjects)
                    continue;
    ...
    connect(ui->addEnt, &QToolButton::clicked, [this] {
        ...
        ui->addEnt->setDisabled(viewer->activeEntityCount() >= SCENEENTITY_COUNT_v5);
        DoAction("Add Entity: " + QString::number(c));
    });
    ...
    connect(ui->alignEnt, &QToolButton::clicked, [this] {
        ui->entityList->blockSignals(true);
        int slot = 0;
        while (slot < viewer->entities.count()){
            SceneEntity *ent = &viewer->entities[slot];
            if (ent->slotID != slot){
                ent->prevSlot = ent->slotID;
                ent->slotID   = slot;
                ent->gameEntitySlot = slot;
                ui->entityList->item(slot)->setText(QString::number(ent->slotID) + ": "
                                                 + viewer->objects[viewer->entities[slot].type].name);
            }
    ...
    connect(scnProp->saveAsStamps, &QPushButton::clicked, [this] {
        QFileDialog filedialog(this, tr("Save Stamps As"), "", tr("RSDKv5 Stamps (*.bin)"));
        filedialog.setAcceptMode(QFileDialog::AcceptSave);
        if (filedialog.exec() == QDialog::Accepted) {
            QString filepath = filedialog.selectedFiles()[0];
            ...
            viewer->stamps.filePath = QFile(filepath).fileName();
            viewer->metadata.stampName = QFileInfo(viewer->stamps.filePath).fileName();
...
void SceneEditorv5::updateType(SceneEntity *entity, byte type, bool keepVals)
{
...
void SceneEditorv5::SetStamp(float x, float y)
{
    ...
    for(int y = 0; y < stamp.size.y; y++){
        for(int x = 0; x < stamp.size.x; x++){
            int tilePosX = viewer->stampFlip.x ? stamp.size.x - x - 1 : x;
            int tilePosY = viewer->stampFlip.y ? stamp.size.y - y - 1 : y;
            ushort tile = stamp.tiles[tilePosX + stamp.size.x * tilePosY];

            if (tile != 0xFFFF) {
                if (viewer->stampFlip.x){
                    bool flipX = Utils::getBit(tile, 10);
                    Utils::setBit(tile, !flipX, 10);
                }
                if (viewer->stampFlip.y){
                    bool flipY = Utils::getBit(tile, 11);
        ...
        if (clipboard || clipboardIDs.count()) {
            switch (clipboardType) {
                ...
                case COPY_ENTITY_SELECT: {
                    if (viewer->activeEntityCount() + clipboardIDs.count() >= SCENEENTITY_COUNT_v5){
                        QMessageBox msgBox =
                            QMessageBox(QMessageBox::Information, "RetroED",
                ...
                case COPY_ENTITY: {
                    if (viewer->activeEntityCount() < SCENEENTITY_COUNT_v5) {
                        SceneEntity *entity = (SceneEntity *)clipboard;
