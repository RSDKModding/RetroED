...
StageconfigEditorv2::StageconfigEditorv2(FormatHelpers::Stageconfig *scf, int globalObjCount,
                                         QWidget *parent)
    : m_stageconfig(scf), QDialog(parent), ui(new Ui::StageconfigEditorv2)
{
    ui->setupUi(this);
    ...
    connect(ui->objList, &QListWidget::itemChanged, [this](QListWidgetItem *item) {
        m_stageconfig->m_objects[ui->objList->row(item)].m_script = item->text();
        m_stageconfig->m_objects[ui->objList->row(item)].m_name   = QFileInfo(item->text()).baseName();
        ...
        ui->objScript->blockSignals(true);
        ui->objScript->setText(m_stageconfig->m_objects[ui->objList->currentRow()].m_script);
        ui->objScript->blockSignals(false);
    });

    connect(ui->objScript, &QLineEdit::textEdited, [this](QString s) {
        m_stageconfig->m_objects[ui->objList->currentRow()].m_script = s;
        m_stageconfig->m_objects[ui->objList->currentRow()].m_name   = QFileInfo(s).baseName();

        ui->objList->item(ui->objList->currentRow())
            ->setText(m_stageconfig->m_objects[ui->objList->currentRow()].m_name);

        ui->objName->blockSignals(true);
        ui->objName->setText(m_stageconfig->m_objects[ui->objList->currentRow()].m_name);
        ui->objName->blockSignals(false);
    });
