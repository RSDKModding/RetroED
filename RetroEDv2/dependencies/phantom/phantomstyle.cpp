...
QColor dividerColor(const QColor &underlying) { return adjustLightness(underlying, -0.05); }
QColor outlineOf(const QPalette &pal) { return adjustLightness(pal.color(QPalette::Window), -0.1); }
QColor gutterColorOf(const QPalette &pal)
{
    return adjustLightness(pal.color(QPalette::Window), -0.03);
}
QColor lightShadeOf(const QColor &underlying) { return adjustLightness(underlying, 0.07); }
QColor darkShadeOf(const QColor &underlying) { return adjustLightness(underlying, -0.07); }
QColor overhangShadowOf(const QColor &underlying) { return adjustLightness(underlying, -0.05); }
QColor sliderGutterShadowOf(const QColor &underlying) { return adjustLightness(underlying, -0.01); }
QColor specularOf(const QColor &underlying) { return adjustLightness(underlying, 0.03); }
...
enum SwatchColor {
    ...
    S_tabFrame             = S_button,
    S_tabFrame_specular    = S_button_specular,
};
}
...
Q_NEVER_INLINE void PhSwatch::loadFromQPalette(const QPalette &pal)
{
    ...
    // brushes[S_highlight_outline] = pal.highlight();
    ...
    for (int i = S_none + 1; i < Num_SwatchColors; ++i) {
        if (i == S_highlight_outline) {
            brushes[i] = QColor(0x61A8D6);
            continue;
        }
        else if (i == S_highlight) {
            brushes[i] = pal.highlight();
        }
        brushes[i] = colors[i];
    }
...
QRect menuItemCheckRect(const MenuItemMetrics &metrics, Qt::LayoutDirection direction, QRect itemRect,
                        bool hasArrow)
{
...
QRect menuItemIconRect(const MenuItemMetrics &metrics, Qt::LayoutDirection direction, QRect itemRect,
                       bool hasArrow)
{
    QRect r = menuItemContentRect(metrics, itemRect, hasArrow);
...
QRect menuItemTextRect(const MenuItemMetrics &metrics, Qt::LayoutDirection direction, QRect itemRect,
...
Q_NEVER_INLINE QPointF calcRadialPos(const QStyleOptionSlider *dial, qreal offset)
{
    ...
    if (dial->maximum == dial->minimum)
        ...
        a = Pi * 3 / 2
            - (currentSliderPosition - dial->minimum) * 2 * Pi / (dial->maximum - dial->minimum);
    else
        a = (Pi * 8
...
int fontMetricsWidth(const QFontMetrics &fontMetrics, const QString &text)
{
    ...
    return fontMetrics.width(text, text.size(), Qt::TextDontClip);
#else
...
Q_NEVER_INLINE void drawArrow(QPainter *p, QRect rect, Qt::ArrowType arrowDirection,
                              const QBrush &brush)
{
                              ...
                              bool allowEnabled = true)
{
    if (rect.isEmpty())
        return;
    using namespace SwatchColors;
    Phantom::drawArrow(painter, rect, type,
...
Q_NEVER_INLINE void fillRectOutline(QPainter *p, QRect rect, QMargins margins, const QBrush &brush)
{
    int x, y, w, h;
...
void fillRectOutline(QPainter *p, QRect rect, int thickness, const QBrush &color)
{
    fillRectOutline(p, rect, QMargins(thickness, thickness, thickness, thickness), color);
}
Q_NEVER_INLINE void fillRectEdges(QPainter *p, QRect rect, Qt::Edges edges, QMargins margins,
                                  const QColor &color)
{
    int x, y, w, h;
    rect.getRect(&x, &y, &w, &h);
...
{
    switch (edge) {
        case Qt::LeftEdge: return rect.translated(-delta, 0);
        case Qt::TopEdge: return rect.translated(0, -delta);
        case Qt::RightEdge: return rect.translated(delta, 0);
        case Qt::BottomEdge: return rect.translated(0, delta);
    }
    return rect;
}
Q_NEVER_INLINE QRect rectFromInnerEdgeWithThickness(QRect rect, Qt::Edge edge, int thickness)
{
    ...
    QRect r;
    switch (edge) {
        case Qt::LeftEdge: r = QRect(x, y, thickness, h); break;
        case Qt::TopEdge: r = QRect(x, y, w, thickness); break;
        case Qt::RightEdge: r = QRect((x + w) - thickness, y, thickness, h); break;
        case Qt::BottomEdge: r = QRect(x, (y + h) - thickness, w, thickness); break;
    }
    return r & rect;
}
Q_NEVER_INLINE void paintSolidRoundRect(QPainter *p, QRect rect, qreal radius, const PhSwatch &swatch,
                                        Swatchy fill)
{
    if (!fill)
        return;
...
Q_NEVER_INLINE void paintBorderedRoundRect(QPainter *p, QRect rect, qreal radius,
                                           const PhSwatch &swatch, Swatchy stroke, Swatchy fill)
{
    ...
    if (radius > 0.5) {
        ...
        p->setBrush(swatch.brush(fill));
        QRectF rf((qreal)rect.x() + 0.5, (qreal)rect.y() + 0.5, (qreal)rect.width() - 1.0,
        ...
        if (aa)
            p->setRenderHint(QPainter::Antialiasing, false);
        if (stroke) {
            fillRectOutline(p, rect, 1, swatch.brush(stroke));
        }
        if (fill) {
...
PhantomStyle::PhantomStyle() : d(new PhantomStylePrivate) { setObjectName(QLatin1String("Phantom")); }
...
void PhantomStyle::drawItemText(QPainter *painter, const QRect &rect, int alignment,
...
void PhantomStyle::drawPrimitive(PrimitiveElement elem, const QStyleOption *option, QPainter *painter,
    ...
    switch ((int)elem) {
        ...
        case PE_FrameTabBarBase: {
            ...
            switch (tbb->shape) {
                ...
                case QTabBar::TriangularNorth: edge = Qt::TopEdge; break;
                case QTabBar::RoundedSouth:
                case QTabBar::TriangularSouth: edge = Qt::BottomEdge; break;
                case QTabBar::RoundedWest:
                case QTabBar::TriangularWest: edge = Qt::LeftEdge; break;
                case QTabBar::RoundedEast:
                case QTabBar::TriangularEast: edge = Qt::RightEdge; break;
            }
        ...
        case PE_IndicatorArrowLeft: {
            ...
            switch (elem) {
                case PE_IndicatorArrowUp: arrow = Qt::UpArrow; break;
                case PE_IndicatorArrowDown: arrow = Qt::DownArrow; break;
                case PE_IndicatorArrowRight: arrow = Qt::RightArrow; break;
                case PE_IndicatorArrowLeft: arrow = Qt::LeftArrow; break;
                default: break;
            }
            ...
            if (arrow == Qt::DownArrow
                && !(bool)qstyleoption_cast<const QStyleOptionToolButton *>(option) && widget) {
        ...
        case PE_PanelButtonTool: {
            ...
            Ph::paintBorderedRoundRect(painter, r.adjusted(1, 1, -1, -1), rounding, swatch, specular,
                                       S_none);
            break;
        }
        ...
        case PE_FrameLineEdit: {
            ...
            if (Ph::OverhangShadows && !hasFocus && isEnabled) {
                ...
                Ph::fillRectEdges(
                    painter, r.adjusted(qRound(rounding / 2) + 1, 1, -(qRound(rounding / 2) + 1), -1),
        ...
        case PE_PanelLineEdit: {
            ...
            Ph::paintSolidRoundRect(painter, option->rect, Ph::LineEdit_Rounding, swatch, S_button);
            save.restore();
...
#endif
            Ph::PSave save(painter);
            Ph::paintBorderedRoundRect(painter, option->rect, Ph::FrameFocusRect_Rounding, swatch,
        ...
        case PE_FrameStatusBarItem: break;
        ...
        default: QCommonStyle::drawPrimitive(elem, option, painter, widget); break;
    }
}

void PhantomStyle::drawControl(ControlElement element, const QStyleOption *option, QPainter *painter,
                               const QWidget *widget) const
{
#ifdef BUILD_WITH_EASY_PROFILER
    ...
    switch (element) {
        ...
        case CE_ProgressBarContents: {
            ...
            if (isIndeterminate || bar->progress > bar->minimum) {
                ...
                Ph::paintBorderedRoundRect(painter, filled, rounding, swatch, S_progressBar_outline,
                                           S_progressBar);
                Ph::paintBorderedRoundRect(painter, filled.adjusted(1, 1, -1, -1), rounding, swatch,
                                           S_progressBar_specular, S_none);
                if (isIndeterminate) {
        ...
        case CE_ProgressBarLabel: {
            ...
            if (r.isEmpty() || !r.isValid())
                break;
            QSize textSize = option->fontMetrics.size(Qt::TextDontClip, bar->text);
            QRect textRect =
        ...
        case CE_MenuBarItem: {
            ...
            painter->fillRect(r, swatch.brush(fill));
        ...
        case CE_MenuItem: {
            ...
            if (isSelected) {
                // fillColor = isSunken ? S_highlight_outline : S_highlight;
      ...
      painter->restore();
        ...
        case CE_TabBarTabShape: {
            ...
            switch (tab->shape) {
                ...
                case QTabBar::RoundedEast:
                    ...
                    edgeTowardNextTab = Qt::BottomEdge;
                    break;
                default: QCommonStyle::drawControl(element, tab, painter, widget); return;
            }
            ...
            if (hasFrame) {
                ...
                if (isSelected) {
                    ...
                    specular =
                        Ph::TabBar_InactiveTabsHaveSpecular ? S_inactiveTabYesFrame_specular : S_none;
                }
            }
            else {
                ...
                if (isSelected) {
                    ...
                    specular =
                        Ph::TabBar_InactiveTabsHaveSpecular ? S_inactiveTabNoFrame_specular : S_none;
                }
            }
            Ph::paintBorderedRoundRect(painter, drawRect, rounding, swatch, S_window_outline,
                                       thisFillColor);
            Ph::paintBorderedRoundRect(painter, drawRect.adjusted(1, 1, -1, -1), rounding, swatch,
                                       specular, S_none);
            painter->restore();
            if (isSelected) {
                ...
                Ph::fillRectEdges(painter, refillRect, edgeAwayNextTab | edgeTowardNextTab, 1,
        ...
        default: QCommonStyle::drawControl(element, option, painter, widget); break;
    }
}

QPalette PhantomStyle::standardPalette() const { return QCommonStyle::standardPalette(); }

void PhantomStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex *option,
    ...
    switch (control) {
        ...
        case CC_SpinBox: {
            ...
            if (spinBox->frame) {
                ...
                Ph::paintBorderedRoundRect(painter,
                                           Ph::expandRect(upDownRect, Ph::oppositeEdge(edge), -1),
        ...
        case CC_TitleBar: {
            ...
            if (titleBar->subControls & SC_TitleBarUnshadeButton
                && (titleBar->titleBarFlags & Qt::WindowShadeButtonHint)) {
                QRect unshadeButtonRect =
                    proxy()->subControlRect(CC_TitleBar, titleBar, SC_TitleBarUnshadeButton, widget);
            ...
            if ((titleBar->subControls & SC_TitleBarSysMenu)
                ...
                if (iconRect.isValid()) {
                    if (!titleBar->icon.isNull()) {
                        ...
                        QPixmap pm =
                            proxy()->standardIcon(SP_TitleBarMenuButton, &tool, widget).pixmap(16, 16);
                        tool.rect = iconRect;
                        painter->save();
        ...
        case CC_ScrollBar: {
            ...
            if (scrollBarGrooveShown) {
                ...
                if (Ph::ScrollbarShadows && scrollBar->subControls & SC_ScrollBarSlider && isEnabled
                    && hasRange) {
                    if (isHorizontal) {
                        r               = scrollBarSlider;
            ...
            if (scrollBar->subControls & SC_ScrollBarSubLine) {
                ...
                Ph::drawArrow(painter, bgRect.adjusted(adj, adj, -adj, -adj), arrowType, swatch,
                              hasRange);
            }
        ...
        case CC_Slider: {
            ...
            if ((option->subControls & SC_SliderGroove) && groove.isValid()) {
                ...
                Ph::paintBorderedRoundRect(painter, groove, Ph::SliderGroove_Rounding, swatch,
                                           outlineColor, gutterColor);
            }
            ...
            if ((option->subControls & SC_SliderHandle)) {
                bool isPressed =
                    option->state & QStyle::State_Sunken && option->activeSubControls & SC_SliderHandle;
                QRect r = handle;
                ...
                Ph::PSave save(painter);
                Ph::paintBorderedRoundRect(painter, r, Ph::SliderHandle_Rounding, swatch, handleOutline,
                                           handleFill);
                r.adjust(1, 1, -1, -1);
        ...
        case CC_Dial:
            ...
            break;
#endif
        default: QCommonStyle::drawComplexControl(control, option, painter, widget); break;
    }
}
                              ...
                              const QWidget *widget) const
{
    int val = -1;
    switch (metric) {
        case PM_SliderTickmarkOffset: val = 4; break;
        case PM_HeaderMargin:
        case PM_ToolTipLabelFrameWidth: val = 2; break;
        case PM_ButtonMargin: val = 6; break;
        case PM_ComboBoxFrameWidth: val = 1; break;
        case PM_ButtonDefaultIndicator:
        case PM_ButtonShiftHorizontal: val = 0; break;
        ...
        case PM_MessageBoxIconSize: val = 48; break;
        case PM_DialogButtonsSeparator:
        case PM_ScrollBarSliderMin: val = 26; break;
        case PM_TitleBarHeight: val = 24; break;
        ...
        case PM_SliderLength: val = 15; break;
        case PM_DockWidgetTitleMargin: val = 1; break;
        case PM_MenuVMargin:
        case PM_MenuHMargin:
        case PM_MenuPanelWidth: val = 0; break;
        case PM_MenuBarItemSpacing: val = 0; break;
        ...
        case PM_MenuBarPanelWidth: val = 0; break;
        case PM_ToolBarSeparatorExtent: val = 9; break;
        case PM_ToolBarHandleExtent: {
            int dotLen = (int)Phantom::dpiScaled(2);
            return dotLen * (3 * 2 - 1);
        }
        case PM_ToolBarItemSpacing: val = 1; break;
        case PM_ToolBarFrameWidth: val = 0; break;
        case PM_ToolBarItemMargin: val = 1; break;
        ...
        case PM_DockWidgetTitleBarButtonMargin: val = 2; break;
        case PM_TitleBarButtonSize: val = 19; break;
        case PM_MaximumDragDistance: return -1; // Do not dpi-scale because the value is magic
        case PM_TabCloseIndicatorWidth:
        case PM_TabCloseIndicatorHeight: val = 16; break;
        ...
        case PM_TabBarTabOverlap: val = 1; break;
        case PM_TabBarBaseOverlap: val = 2; break;
        case PM_TabBarIconSize: {
            if (!widget)
                break;
            return widget->fontMetrics().height();
        }
        case PM_SubMenuOverlap: val = 0; break;
        case PM_DockWidgetHandleExtent:
        case PM_SplitterWidth: val = 5; break;
        ...
        case PM_ExclusiveIndicatorWidth:
            ...
            val = 14;
            break;
        case PM_ScrollView_ScrollBarSpacing: val = 0; break;
        case PM_ScrollView_ScrollBarOverlap: val = 0; break;
...
QSize PhantomStyle::sizeFromContents(ContentsType type, const QStyleOption *option, const QSize &size,
                                     const QWidget *widget) const
{
    namespace Ph = Phantom;
    ...
    switch (type) {
        ...
        case CT_HeaderSection: {
            ...
            QSize txt     = hdr->fontMetrics.size(Qt::TextSingleLine | Qt::TextDontClip, hdr->text);
            QSize sz;
    ...
    switch (type) {
        case CT_PushButton: {
            ...
            int hpad = (int)((qreal)fmheight * Phantom::PushButton_HorizontalPaddingFontHeightRatio);
        ...
        case CT_ToolButton: newSize += QSize(2, 2); break;
        case CT_ComboBox: {
            newSize += QSize(0, 3);
#if QT_CONFIG(combobox)
            auto cb = qstyleoption_cast<const QStyleOptionComboBox *>(option);
        ...
        case CT_SpinBox:
            ...
            break;
        case CT_SizeGrip: newSize += QSize(4, 4); break;
        case CT_MdiControls: newSize -= QSize(1, 0); break;
        default: break;
    }
    return newSize;
}
...
QRect PhantomStyle::subControlRect(ComplexControl control, const QStyleOptionComplex *option,
    ...
    switch (control) {
        ...
        case CC_SpinBox: {
            ...
            switch (subControl) {
                ...
                case SC_SpinBoxEditField:
                    ...
                    break;
                case SC_SpinBoxFrame: rect = spinbox->rect; break;
                default: break;
            }
            ...
            switch (subControl) {
                ...
                case SC_GroupBoxLabel: {
                    ...
                    switch (groupBox->textAlignment & Qt::AlignHorizontal_Mask) {
                        case Qt::AlignHCenter: x += (option->rect.width() - contentWidth) / 2; break;
                        case Qt::AlignRight: x += option->rect.width() - contentWidth; break;
                        default: break;
                    }
                    int w, h;
        ...
        case CC_ComboBox: {
            ...
            if (dim < 1)
                return QRect();
            switch (subControl) {
                case SC_ComboBoxFrame: return cb->rect;
...
void PhantomStyle::drawItemPixmap(QPainter *painter, const QRect &rect, int alignment,
...
QPixmap PhantomStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap &pixmap,
                                          const QStyleOption *opt) const
{
    return QCommonStyle::generatedIconPixmap(iconMode, pixmap, opt);
}

int PhantomStyle::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget,
                            QStyleHintReturn *returnData) const
{
    switch (hint) {
        ...
        case SH_Menu_SupportsSections: return 0;
#ifndef Q_OS_MAC
        case SH_MenuBar_AltKeyNavigation: return 1;
#endif
#if defined(QT_PLATFORM_UIKIT)
        case SH_ComboBox_UseNativePopup: return 1;
#endif
        ...
        case SH_ItemView_MovementWithoutUpdatingSelection: return 1;
#if QT_CONFIG(itemviews)
        ...
        case SH_RubberBand_Mask: return 0;
...
#if QT_CONFIG(wizard)
        case SH_WizardStyle: return QWizard::ClassicStyle;
#endif
        case SH_Menu_SubMenuPopupDelay:
            // Returning 0 will break sloppy submenus even if they're enabled
            return 10;
        case SH_Menu_SloppySubMenus: return true;
        case SH_Menu_SubMenuSloppyCloseTimeout: return 500;
        case SH_Menu_SubMenuDontStartSloppyOnLeave: return 1;
        case SH_Menu_SubMenuSloppySelectOtherActions: return 1;
        case SH_Menu_SubMenuUniDirection: return 1;
        case SH_Menu_SubMenuUniDirectionFailCount: return 1;
        case SH_Menu_SubMenuResetWhenReenteringParent: return 0;
#ifdef Q_OS_MAC
        case SH_Menu_FlashTriggeredItem: return 1;
        case SH_Menu_FadeOutOnHide: return 0;
#endif
        case SH_WindowFrame_Mask: return 0;
        ...
        case SH_Widget_Animate: return 0;
        default: break;
    }
