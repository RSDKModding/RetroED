...
struct RingEditor {
    ...
    RingEditor(double hue_diff, bool editable, int symmetric_to = -1, int opposite_to = -1)
        : hue_diff(hue_diff), editable(editable), symmetric_to(symmetric_to), opposite_to(opposite_to)
    {
    }
};
...
public:
    ...
    Private(ColorWheel *widget)
        : w(widget), hue(0), sat(0), val(0), wheel_width(20), mouse_status(Nothing),
    ...
    void init_buffer(QSize size)
    {
        ...
        inner_selector = QImage(reinterpret_cast<uchar *>(inner_selector_buffer.data()), size.width(),
...
QColor ColorWheel::color() const { return p->color_from(p->hue, p->sat, p->val, 1); }
...
unsigned int ColorWheel::harmonyCount() const { return 1 + p->ring_editors.size(); }

QSize ColorWheel::sizeHint() const { return QSize(p->wheel_width * 5, p->wheel_width * 5); }
...
void ColorWheel::paintEvent(QPaintEvent *)
{
    ...
    if (gradient_hue.stops().size() < hue_stops) {
        for (double a = 0; a < 1.0; a += 1.0 / (hue_stops - 1)) {
            gradient_hue.setColorAt(a, p->rainbow_from_hue(a));
        }
        gradient_hue.setColorAt(1, p->rainbow_from_hue(0));
    }

    painter.translate(p->outer_radius(), p->outer_radius());

    painter.setPen(Qt::NoPen);
    painter.setBrush(QBrush(gradient_hue));

    QPainterPath ring;
    ring.addEllipse(QPointF(0, 0), p->outer_radius(), p->outer_radius());
    ring.addEllipse(QPointF(0, 0), p->inner_radius(), p->inner_radius());
    painter.drawPath(ring);

    //    painter.drawEllipse(QPointF(0,0),p->inner_radius(),p->inner_radius());
...
unsigned ColorWheel::addOppositeHarmony(unsigned relative_to)
{
    ...
    p->ring_editors.emplace_back(normalize(0.5 + relative.hue_diff), relative.editable, -1,
                                 relative_to);
    p->apply_harmonies();
    return count;
}
