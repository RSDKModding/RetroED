...
ColorPalette::ColorPalette(const QVector<QColor> &colors, const QString &name, int columns)
    : p(new Private)
{
    setName(name);
    setColumns(columns);
...
ColorPalette::ColorPalette(const QVector<QPair<QColor, QString>> &colors, const QString &name,
                           int columns)
{
    setName(name);
    setColumns(columns);
...
ColorPalette::ColorPalette(ColorPalette &&other) : QObject(), p(other.p) { other.p = nullptr; }
...
QVector<QPair<QColor, QString>> ColorPalette::colors() const { return p->colors; }
...
QPixmap ColorPalette::preview(const QSize &size, const QColor &background) const
{
    ...
    for (int y = 0, i = 0; y < rows && i < count; y++) {
        for (int x = 0; x < columns && i < count; x++, i++) {
            painter.fillRect(QRectF(x * color_size.width(), y * color_size.height(), color_size.width(),
                                    color_size.height()),
                             p->colors[i].first);
        }
    }
...
QVector<QRgb> ColorPalette::colorTable() const
{
    QVector<QRgb> out;
    out.reserve(p->colors.size());
    for (const auto &color_pair : p->colors) out.push_back(color_pair.first.rgba());
    return out;
}
