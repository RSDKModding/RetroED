...
public:
    ...
    Private(Swatch *owner)
        : selected(-1), color_size(16, 16), size_policy(Hint), border(Qt::black, 1), forced_rows(0),
          forced_columns(0), readonly(false), drag_index(-1), drop_index(-1), drop_overwrite(false),
          owner(owner)
    {
    }
    ...
    QRectF indexRect(int index, const QSize &rowcols, const QSizeF &color_size)
    {
        ...
        return QRectF(index % rowcols.width() * color_size.width(),
                      index / rowcols.width() * color_size.height(), color_size.width(),
                      color_size.height());
    }
    /**
     * \brief Rectangle corresponding to the color at the given index
...
QSize Swatch::sizeHint() const
{
    ...
    return QSize(p->color_size.width() * rowcols.width(), p->color_size.height() * rowcols.height());
}
...
int Swatch::indexAt(const QPoint &pt)
{
    ...
    QPoint point(qBound<int>(0, pt.x() / color_size.width(), rowcols.width() - 1),
...
QColor Swatch::colorAt(const QPoint &pt) { return p->palette.colorAt(indexAt(pt)); }
...
Swatch::ColorSizePolicy Swatch::colorSizePolicy() const { return p->size_policy; }
