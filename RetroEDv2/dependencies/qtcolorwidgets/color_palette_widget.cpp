...
public:
    ...
    bool hasSelectedPalette() { return model && palette_list->currentIndex() != -1; }

    const ColorPalette &selectedPalette() { return model->palette(palette_list->currentIndex()); }
...
ColorPaletteWidget::ColorPaletteWidget(QWidget *parent) : QWidget(parent), p(new Private)
{
    p->setupUi(this);
    ...
    connect(p->swatch, &Swatch::colorSelected, this,
            (void (ColorPaletteWidget::*)(const QColor &)) & ColorPaletteWidget::currentColorChanged);
    connect(p->swatch, &Swatch::selectedChanged, this,
            (void (ColorPaletteWidget::*)(int)) & ColorPaletteWidget::currentColorChanged);
    ...
    connect(&p->swatch->palette(), &ColorPalette::dirtyChanged, p->button_palette_save,
            &QWidget::setEnabled);
    connect(&p->swatch->palette(), &ColorPalette::dirtyChanged, p->button_palette_revert,
            &QWidget::setEnabled);

    connect(p->palette_list, (void (QComboBox::*)(int)) & QComboBox::currentIndexChanged, this,
    ...
    connect(p->button_palette_duplicate, &QAbstractButton::clicked, [this]() {
        if (p->hasSelectedPalette()) {
            ...
            bool ok      = false;
            QString name = QInputDialog::getText(this, tr("New Palette"), tr("Name"), QLineEdit::Normal,
                                                 new_palette.name(), &ok);
            if (ok) {
    ...
    connect(p->button_palette_open, &QAbstractButton::clicked, [this, image_formats]() {
        if (p->model) {
            ...
            QStringList file_formats = QStringList() << tr("GIMP Palettes (*.gpl)")
...
const ColorPalette &ColorPaletteWidget::currentPalette() const { return p->swatch->palette(); }
...
int ColorPaletteWidget::forcedColumns() const { return p->swatch->forcedColumns(); }

bool ColorPaletteWidget::readOnly() const { return p->read_only; }

QColor ColorPaletteWidget::currentColor() const { return p->swatch->selectedColor(); }
...
void ColorPaletteWidget::setColorSize(const QSize &colorSize) { p->swatch->setColorSize(colorSize); }
...
void ColorPaletteWidget::setBorder(const QPen &border) { p->swatch->setBorder(border); }

void ColorPaletteWidget::setForcedRows(int forcedRows) { p->swatch->setForcedRows(forcedRows); }
...
int ColorPaletteWidget::currentRow() const { return p->palette_list->currentIndex(); }

void ColorPaletteWidget::setCurrentRow(int row) { p->palette_list->setCurrentIndex(row); }
