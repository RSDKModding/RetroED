...
public:
    ...
    QList<ColorPalette>::iterator find(const QString &name)
    {
        return std::find_if(palettes.begin(), palettes.end(),
                            [&name](const ColorPalette &palette) { return palette.name() == name; });
    }
    ...
    bool save(ColorPalette &palette, const QString &suggested_filename = QString())
    {
        ...
        return attemptSave(palette, save_dir.absoluteFilePath(
...
QVariant ColorPaletteModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role) {
        case Qt::DisplayRole: return palette.name();
        case Qt::DecorationRole: return palette.preview(p->icon_size);
        case Qt::ToolTipRole: return tr("%1 (%2 colors)").arg(palette.name()).arg(palette.count());
    }
...
const ColorPalette &ColorPaletteModel::palette(const QString &name) const { return *p->find(name); }

const ColorPalette &ColorPaletteModel::palette(int index) const { return p->palettes[index]; }
