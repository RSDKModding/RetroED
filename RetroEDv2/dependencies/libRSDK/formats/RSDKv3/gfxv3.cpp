...
void rle_writeRSDKv3(Writer writer, int pixel, int count, bool dcGFX)
{
    if (count <= 2) {
...
void RSDKv3::GFX::write(Writer &writer, bool dcGFX)
{
    ...
    for (int x = 0; x < height * width; ++x) {
        if ((byte)pixels[x] != p && x > 0) {
            rle_writeRSDKv3(writer, p, cnt, dcGFX);
            cnt = 0;
        }
        p = (byte)pixels[x];
        ++cnt;
...
void RSDKv3::GFX::importImage(QImage image)
{
...
void RSDKv3::GFX::importImage(FormatHelpers::Gif image)
{
...
FormatHelpers::Gif RSDKv3::GFX::exportGif()
{
